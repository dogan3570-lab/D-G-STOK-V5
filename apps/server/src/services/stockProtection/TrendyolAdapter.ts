// ==================== TRENDYOL ADAPTER V3 ====================
// StockProtection → MarketplaceAdapter → TrendyolAdapter → Trendyol API
// Strateji: stok=0 gönder (listing pasif olur)
...
export class TrendyolAdapter implements MarketplaceAdapter {
  ...
  async updateStock(marketplaceKey: string, _productId: string, sku: string, stock: number): Promise<AdapterResult> {
    ...
    try {
      ...
      try { if (mp.settings) { const s = JSON.parse(mp.settings); supplierId = s.sellerId || s.supplierId; } } catch {}
      ...
      const sid = supplierId || '2738';
      const url = `${baseUrl}/suppliers/${sid}/products/stock-update`;

      const auth = Buffer.from(`${apiKey}:${apiSecret}`).toString('base64');
      const body = JSON.stringify({
        items: [{ barcode: sku, quantity: stock }]
      });
      ...
      if (response.ok) {
        return {
          success: true,
          message: `Trendyol stock updated for SKU ${sku}: ${stock}`,
          marketplaceKey,
          durationMs,
        ...
        return {
          success: false,
          message: `Trendyol API error: ${response.status}`,
          marketplaceKey,
          durationMs,
          httpStatus: response.status,
          apiResponse: responseBody,
