// ==================== Hepsiburada ADAPTER V3 ====================
// StockProtection → MarketplaceAdapter → HepsiburadaAdapter → HB API
// Strateji: listing pasif (HB API üzerinden)
...
export class HepsiburadaAdapter implements MarketplaceAdapter {
  ...
  async updateStock(marketplaceKey: string, _productId: string, sku: string, stock: number): Promise<AdapterResult> {
    ...
    try {
      ...
      try { if (mp.settings) { const s = JSON.parse(mp.settings); merchantId = s.merchantId || s.sellerId; } } catch {}
      if (!apiKey || !apiSecret) throw new Error('Hepsiburada API Key veya Secret eksik');

      const baseUrl = mp.apiUrl || 'https://api.hepsiburada.com';
      const url = `${baseUrl}/v2/product/stock`;

      const auth = Buffer.from(`${apiKey}:${apiSecret}`).toString('base64');
      const body = JSON.stringify({
        items: [{ merchantSku: sku, quantity: stock }]
      });
      ...
      if (response.ok) {
        return {
          success: true,
          message: `Hepsiburada stock updated for SKU ${sku}: ${stock}`,
          marketplaceKey,
          durationMs,
        ...
        return {
          success: false,
          message: `Hepsiburada API error: ${response.status}`,
          marketplaceKey,
          durationMs,
          httpStatus: response.status,
          apiResponse: responseBody,
