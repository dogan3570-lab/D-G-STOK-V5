// ==================== N11 ADAPTER V3 ====================
// StockProtection → MarketplaceAdapter → N11Adapter → N11 API
// Strateji: stok güncelle (N11 API üzerinden)
...
export class N11Adapter implements MarketplaceAdapter {
  ...
  async updateStock(marketplaceKey: string, _productId: string, sku: string, stock: number): Promise<AdapterResult> {
    ...
    try {
      ...
      if (!apiKey || !apiSecret) throw new Error('N11 API Key veya Secret eksik');

      const baseUrl = mp.apiUrl || 'https://api.n11.com';
      const url = `${baseUrl}/ws/product/updateStock`;
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
    <productStockUpdate>
      ...
      if (response.ok && !responseBody.includes('error')) {
        return {
          success: true,
          message: `N11 stock updated for SKU ${sku}: ${stock}`,
          marketplaceKey,
          durationMs,
        ...
        return {
          success: false,
          message: `N11 API error: ${response.status}`,
          marketplaceKey,
          durationMs,
          httpStatus: response.status,
          apiResponse: responseBody,
  ...
  async health(marketplaceKey: string): Promise<AdapterHealthResult> {
    ...
    try {
      ...
      const baseUrl = mp.apiUrl || 'https://api.n11.com';
      const response = await fetch(`${baseUrl}/ws/product/health`, { method: 'GET' });
