...
export default function EditProductPage() {
  ...
  const loadProduct = async () => {
    try {
      const data = await api.request(`/products/${params.id}`);
      ...
      setForm({
        ...
        criticalStock: String(product.criticalStock ?? '5'),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.request(`/products/${params.id}`, {
        method: 'PATCH',
        body: JSON.stringify({
          sku: form.sku.trim(),
    ...
    return (
      <main className="min-h-screen bg-gray-50 pt-24">
        <div className="max-w-3xl mx-auto px-6 py-24 text-center">
          ...
          <h1 className="text-2xl font-light text-gray-900 mb-2">Ürün Bulunamadı</h1>
          <p className="text-gray-500 mb-6">Bu ürün mevcut değil veya silinmiş olabilir.</p>
          <Link
            href="/admin/products"
    ...
    <main className="min-h-screen bg-gray-50 pt-24">
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            <h1 className="text-3xl font-light text-gray-900">Ürün Düzenle</h1>
            <p className="text-sm text-gray-500 mt-1">{form.sku} — {form.name}</p>
          </div>
        </div>
